...
function setRepoPolicy(key,value){const r=store.repositories[key];if(!r)return;r.syncPolicy=value;store.save().then(()=>{toast(value==='auto'?'已启用自动同步':'同步策略已保存');renderRepositories()}).catch(e=>toast('同步策略保存失败：'+e.message,'error'))}
function renderRepositories(){const v=$('sync-repositories');if(!v)return;const repos=Object.values(store.repositories);if(!repos.length){v.innerHTML='';return}v.innerHTML='<section class="sync-repositories panel"><div class="panel-head"><h2>Repository Management</h2><span>'+repos.length+' 个已注册仓库</span></div><div class="sync-repo-list">'+repos.map(r=>{const key=store.repoKey(r),docs=Object.values(r.documents||{}),mapped=docs.filter(x=>x.mapped).length,pending=S.results.filter(x=>x.owner===r.owner&&x.repo===r.repo&&['changed','conflict','local-modified','unmapped','source-deleted','error'].includes(x.status)).length;return '<article class="sync-repo-card"><div><span class="sync-kicker">GITHUB REPOSITORY</span><h3>'+esc(r.owner+'/'+r.repo)+'</h3><p>branch: '+esc(r.branch||'main')+' · '+docs.length+' 个 Registry 文档 · '+mapped+' 个已映射 · '+pending+' 个待处理</p></div><div class="sync-repo-card-meta"><span class="sync-status '+(r.enabled===false?'new':'unchanged')+'">'+(r.enabled===false?'已暂停':'运行中')+'</span><label class="sync-policy">同步策略 <select data-repo-policy="'+esc(key)+'"><option value="manual" '+(r.syncPolicy!=='check'&&r.syncPolicy!=='auto'?'selected':'')+'>手动</option><option value="check" '+(r.syncPolicy==='check'?'selected':'')+'>检查时提示</option><option value="auto" '+(r.syncPolicy==='auto'?'selected':'')+'>自动同步</option></select></label><button class="secondary" data-repo-scan="'+esc(key)+'">重新扫描</button><button class="text-btn" data-repo-toggle="'+esc(key)+'">'+(r.enabled===false?'恢复':'暂停')+'</button></div></article>'}).join('')+'</div></section>';v.querySelectorAll('[data-repo-scan]').forEach(b=>b.onclick=()=>rescanRepository(b.dataset.repoScan));v.querySelectorAll('[data-repo-toggle]').forEach(b=>b.onclick=()=>toggleRepository(b.dataset.repoToggle));v.querySelectorAll('[data-repo-policy]').forEach(b=>b.onchange=e=>setRepoPolicy(b.dataset.repoPolicy,e.target.value))}
const baseCheckAll=checkAll;
checkAll=async function(){await baseCheckAll();const auto=Object.values(store.repositories).filter(r=>r.enabled!==false&&r.syncPolicy==='auto');for(const repo of auto){const keys=new Set(S.results.filter(x=>x.owner===repo.owner&&x.repo===repo.repo&&x.status==='changed').map(x=>x.id));for(const m of store.mappings.filter(x=>keys.has(x.id)))await syncOne(m.id)}}
