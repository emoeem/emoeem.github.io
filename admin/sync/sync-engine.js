...
export function createSyncEngine({api,store}){
  ...
  async function inspect(m){
    const src=await getSource(m),old=await getTarget(m.target);
    if(old){
      const parsed=parseFrontMatter(api.decode(old.content));
      if(parsed.meta.syncCommit===src.commit)return {...m,status:'unchanged',commit:src.commit,date:src.date};
      if(parsed.meta.syncContentHash){
        const h=await sha256(parsed.body);
        if(h!==parsed.meta.syncContentHash)return {...m,status:'local-modified',commit:src.commit,date:src.date};
      }
    }
    return {...m,status:'changed',commit:src.commit,date:src.date};
  }
  async function syncOne(m,force=false){
    const src=await getSource(m),old=await getTarget(m.target),oldParsed=old?parseFrontMatter(api.decode(old.content)):null;
    if(oldParsed?.meta.syncCommit===src.commit)return {...m,status:'unchanged',commit:src.commit,date:src.date};
    if(oldParsed?.meta.syncContentHash&&!force){
      const h=await sha256(oldParsed.body);
      if(h!==oldParsed.meta.syncContentHash)return {...m,status:'local-modified',commit:src.commit,date:src.date};
    }
    const parsed=parseFrontMatter(src.content);
    const refs=parseReferences(parsed.body);
    let body=parsed.body; for(const link of refs.links){if(!isMarkdownReference(link.url)||(link.url.startsWith('http://')||link.url.startsWith('https://')))continue; const tp=resolveRelative(m.path,link.url.split('#')[0].split('?')[0]),tm=store.find(m.owner,m.repo,tp); if(!tm)continue; const tf=await getTarget(tm.target); if(!tf)continue; const tfm=parseFrontMatter(api.decode(tf.content)),d=String(tfm.meta.date||'').slice(0,10).replaceAll('-','/'),u=tfm.meta.permalink||('/'+d+'/'+slug(tfm.meta.title||tm.name)+'/'); body=body.split(link.url).join(u+(link.url.includes('#')?'#'+link.url.split('#')[1]:'')); }
    for(const img of refs.images){if(!isRelativeAsset(img.url))continue; try{const ap=resolveRelative(m.path,img.url.split('#')[0].split('?')[0]); const ar=await api.request('repos/'+m.owner+'/'+m.repo+'/contents/'+ap+'?ref='+encodeURIComponent(m.branch)); if(!ar?.content)continue; const at='source/images/github/'+m.owner+'/'+m.repo+'/'+ap; let existing=null; try{existing=await api.request('repos/emoeem/blog-source/contents/'+at)}catch{} const put={message:'assets: sync '+ap,content:ar.content,branch:'main'}; if(existing?.sha)put.sha=existing.sha; await api.request('repos/emoeem/blog-source/contents/'+at,'PUT',put); body=body.split(img.url).join('/images/github/'+m.owner+'/'+m.repo+'/'+ap)}catch{}}
    const meta={...(oldParsed?.meta||{}),...(m.metadata||{}),
      title:m.metadata?.title||parsed.meta.title||m.name,
      syncSource:m.owner+'/'+m.repo,syncPath:m.path,syncBranch:m.branch,
      syncCommit:src.commit,lastVerified:src.date,
      syncContentHash:await sha256(body),syncLinkCount:refs.links.length,syncImageCount:refs.images.length};
    const content=dumpFrontMatter(meta)+body.replace(/^\n+/,'');
    const put={message:'sync: '+meta.title,content:api.encode(content),branch:'main'};
    if(old)put.sha=old.sha;
    await api.request('repos/emoeem/blog-source/contents/'+m.target,'PUT',put);
  ...
  return {inspect,syncOne,syncAll,defaultTarget};
}
